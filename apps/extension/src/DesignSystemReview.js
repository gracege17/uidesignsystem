...
export default function DesignSystemReview({ result, layout = "stacked", theme = "light" }) {
    ...
    return (_jsxs("div", { className: "space-y-12", children: [_jsx(SectionShell, { title: "Overview", subtitle: "Key extracted foundations and primary signals.", theme: theme, children: _jsx(OverviewSection, { summary: summary, theme: theme, copied: copiedKey === "everything", onCopy: () => void copySection("everything") }) }), _jsx(SectionShell, { title: "Color Styles", subtitle: "Starter color styles extracted from the page.", theme: theme, children: _jsx(ColorSection, { tokens: result.tokens, summary: summary, theme: theme, copied: copiedKey === "color", onCopy: () => void copySection("color") }) }), _jsx(SectionShell, { title: "Typography", subtitle: "Likely display, heading, and body styles.", theme: theme, children: _jsx(TypographySection, { tokens: result.tokens, summary: summary, theme: theme, copied: copiedKey === "typography", onCopy: () => void copySection("typography") }) }), _jsx(SectionShell, { title: "Grid & Layout", subtitle: "Common layout patterns and spacing primitives.", theme: theme, children: _jsx(LayoutSection, { components: result.components, summary: summary, theme: theme, copied: copiedKey === "layout", onCopy: () => void copySection("layout") }) }), _jsx(SectionShell, { title: "Components", subtitle: "Top reusable component families extracted from the page.", theme: theme, children: _jsx(ComponentsSection, { result: result, summary: summary, theme: theme, copied: copiedKey === "components", onCopy: () => void copySection("components") }) })] }));
}
function SplitSection({ tab, result, summary, theme, copiedKey, onCopy }) {
    if (tab === "overview") {
        return (_jsx(SectionShell, { title: "Overview", subtitle: "Start here to identify the main design-system signals.", theme: theme, children: _jsx(OverviewSection, { summary: summary, theme: theme, copied: copiedKey === "everything", onCopy: () => void onCopy("everything") }) }));
    }
    if (tab === "color") {
        return (_jsx(SectionShell, { title: "Color Styles", subtitle: "Grouped color styles with primary and supporting tokens.", theme: theme, children: _jsx(ColorSection, { tokens: result.tokens, summary: summary, theme: theme, copied: copiedKey === "color", onCopy: () => void onCopy("color") }) }));
    }
    if (tab === "typography") {
        return (_jsx(SectionShell, { title: "Typography", subtitle: "Likely display, heading, and body specimens.", theme: theme, children: _jsx(TypographySection, { tokens: result.tokens, summary: summary, theme: theme, copied: copiedKey === "typography", onCopy: () => void onCopy("typography") }) }));
    }
    if (tab === "layout") {
        return (_jsx(SectionShell, { title: "Grid & Layout", subtitle: "Visual layout primitives instead of raw auto-layout metadata.", theme: theme, children: _jsx(LayoutSection, { components: result.components, summary: summary, theme: theme, copied: copiedKey === "layout", onCopy: () => void onCopy("layout") }) }));
    }
    return (_jsx(SectionShell, { title: "Components", subtitle: "Curated gallery of the most repeated component families.", theme: theme, children: _jsx(ComponentsSection, { result: result, summary: summary, theme: theme, copied: copiedKey === "components", onCopy: () => void onCopy("components") }) }));
}
function OverviewSection({ summary, theme, copied, onCopy }) {
    const ui = getThemeClasses(theme);
    return (_jsxs("div", { className: "space-y-8", children: [_jsx(HeaderActions, { label: "Copy Everything", copied: copied, onCopy: onCopy, theme: theme }), _jsx("div", { className: `border-t pt-8 ${ui.rule}`, children: _jsxs("div", { className: "grid gap-4 md:grid-cols-3", children: [_jsx(OverviewCard, { label: "Primary Color", value: summary.primaryColor?.name ?? "Not found", detail: summary.primaryColor?.value ?? "No dominant color yet", theme: theme }), _jsx(OverviewCard, { label: "Main Typeface", value: summary.mainTypography?.fontFamily ?? "Not found", detail: summary.mainTypography ? `${summary.mainTypography.fontSize}px / ${summary.mainTypography.lineHeight}px` : "No dominant text style yet", theme: theme }), _jsx(OverviewCard, { label: "Top Family", value: summary.componentFamilies[0]?.type ?? "Not found", detail: summary.componentFamilies[0] ? `${summary.componentFamilies[0].count} instances` : "No repeated family yet", theme: theme })] }) }), _jsxs("div", { className: `grid gap-4 md:grid-cols-2`, children: [_jsxs("div", { className: `p-6 ${ui.softPanel}`, children: [_jsx("p", { className: `text-[11px] uppercase tracking-[0.22em] ${ui.mutedText}`, children: "Designer Questions" }), _jsxs("div", { className: `mt-4 space-y-3 text-sm ${ui.bodyText}`, children: [_jsxs("p", { children: ["Primary color: ", _jsx("span", { className: ui.strongText, children: summary.primaryColor?.value ?? "Unknown" })] }), _jsxs("p", { children: ["H1 candidate: ", _jsx("span", { className: ui.strongText, children: summary.h1?.name ?? "Unknown" })] }), _jsxs("p", { children: ["Body style: ", _jsx("span", { className: ui.strongText, children: summary.body?.name ?? "Unknown" })] }), _jsxs("p", { children: ["Main component families: ", _jsx("span", { className: ui.strongText, children: summary.componentFamilies.slice(0, 3).map((family) => family.type).join(", ") || "Unknown" })] })] })] }), _jsxs("div", { className: `p-6 ${ui.softPanel}`, children: [_jsx("p", { className: `text-[11px] uppercase tracking-[0.22em] ${ui.mutedText}`, children: "System Snapshot" }), _jsxs("div", { className: "mt-4 grid grid-cols-2 gap-3", children: [_jsx(MiniMetric, { label: "Colors", value: summary.counts.colors, theme: theme }), _jsx(MiniMetric, { label: "Typography", value: summary.counts.typography, theme: theme }), _jsx(MiniMetric, { label: "Layouts", value: summary.counts.layouts, theme: theme }), _jsx(MiniMetric, { label: "Components", value: summary.counts.components, theme: theme })] })] })] })] }));
}
function ColorSection({ tokens, summary, theme, copied, onCopy }) {
    const ui = getThemeClasses(theme);
    const groups = [
    ...
    return (_jsxs("div", { className: "space-y-10", children: [_jsx(HeaderActions, { label: "Copy Color", copied: copied, onCopy: onCopy, theme: theme }), groups.map((group) => (_jsxs("section", { className: "grid grid-cols-[88px_minmax(0,1fr)] gap-8", children: [_jsx("div", { className: `pt-3 text-sm font-medium ${ui.subtleText}`, children: group.label }), _jsx("div", { className: "grid gap-4 sm:grid-cols-2 xl:grid-cols-4", children: group.items.map((token) => (_jsxs("div", { className: `${ui.softPanel} overflow-hidden`, children: [_jsx("div", { className: `h-20 ${ui.rule}`, style: { backgroundColor: token.value } }), _jsxs("div", { className: "p-4", children: [_jsx("p", { className: `text-sm font-semibold ${ui.headingText}`, children: token.name }), _jsx("p", { className: `mt-1 text-xs ${ui.bodyText}`, children: token.value }), _jsx("p", { className: `mt-2 truncate text-[11px] ${ui.mutedText}`, children: token.source })] })] }, token.id))) })] }, group.label))), groups.length === 0 ? (_jsx(EmptyState, { message: "No clear color groups were found.", theme: theme })) : null] }));
}
function TypographySection({ tokens, summary, theme, copied, onCopy }) {
    const ui = getThemeClasses(theme);
    const scale = [summary.h1, summary.h2, summary.h3, summary.body, summary.caption].filter((token) => Boolean(token));
    return (_jsxs("div", { className: "space-y-8", children: [_jsx(HeaderActions, { label: "Copy Typography", copied: copied, onCopy: onCopy, theme: theme }), _jsxs("div", { className: `p-8 ${ui.heroPanel}`, children: [_jsx("p", { className: `text-[11px] uppercase tracking-[0.22em] ${ui.heroMetaText}`, children: summary.mainTypography?.fontFamily ?? "Typeface" }), _jsx("h2", { className: `mt-3 text-5xl font-semibold tracking-tight ${ui.heroHeadingText}`, children: summary.mainTypography?.fontFamily ?? "Typography" }), _jsxs("div", { className: `mt-6 space-y-2 text-sm ${ui.heroBodyText}`, children: [_jsx("p", { children: "ABCDEFGHIJKLMNOPQRSTUVWXYZ" }), _jsx("p", { children: "abcdefghijklmnopqrstuvwxyz" }), _jsx("p", { children: "0123456789" }), _jsx("p", { children: "!@#$%^&*()" })] })] }), _jsx("div", { className: "space-y-8", children: scale.map((token) => (_jsxs("div", { className: `border-t pt-6 ${ui.rule}`, children: [_jsxs("div", { className: `mb-4 grid grid-cols-[minmax(0,1fr)_100px_100px] gap-4 text-[11px] uppercase tracking-[0.18em] ${ui.mutedText}`, children: [_jsx("span", { children: token.name }), _jsx("span", { children: "Font Size" }), _jsx("span", { children: "Line Height" })] }), _jsxs("div", { className: "grid grid-cols-[minmax(0,1fr)_100px_100px] gap-4", children: [_jsx("p", { className: `pr-6 ${ui.headingText}`, style: {
...
function SectionShell({ title, subtitle, children, theme }) {
    const ui = getThemeClasses(theme);
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: `border-t pt-6 ${ui.rule}`, children: [_jsx("h1", { className: `text-5xl font-semibold tracking-tight ${ui.headingText}`, children: title }), _jsx("p", { className: `mt-3 text-sm leading-6 ${ui.mutedText}`, children: subtitle })] }), children] }));
}
...
function HeaderActions({ label, copied, onCopy, theme }) {
    const ui = getThemeClasses(theme);
    return (_jsx("div", { className: "flex justify-end", children: _jsx("button", { type: "button", onClick: onCopy, className: `rounded-full border px-4 py-2 text-sm transition ${ui.copyButton}`, children: copied ? "Copied" : label }) }));
}
...
function buildTypographyCopy(summary) {
    ...
    if (summary.mainTypography)
        lines.push(`- Main typeface: ${summary.mainTypography.fontFamily}`);
    if (summary.h1)
